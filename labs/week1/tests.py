...
def test_sorting(results_dict):
    """
    Checks lab1.sorting(my_list) returns a sorted list
    """
    unsorted_list=[12,7,1,9,15,18,21,3]
    print("")
    print("Testing lab1.sorting()")
    message = "----"
    try:
        result=lab1.sorting(unsorted_list)
        if not isinstance(result, list):
            message+="Did not return a sorted list -Fail"
            test_failed(results_dict)
        #now check if the list is sorted
            ...
            if result==unsorted_list:
                message+='\n----Result is a perfectly sorted list -Pass'
                test_passed(results_dict)
            else:
                message+='\n----Result is not a sorted list -Fail'
                test_failed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.check_is_less_than function - Fail'
        test_failed(results_dict)
...
def test_perfect_num(results_dict):
    """
    Perfect number is a positive integer that is equal to the sum of all its
    ...
    """
    print("")
    print("Testing lab1.perfect_num()")
    message = "----"
    try:
        result=lab1.perfect_num(28)
        if not isinstance(result, bool):
            message+="Did not return a boolean value-Fail"
            test_failed(results_dict)
        ...
        #now check if the result is true or not
            ...
            test_passed(results_dict)
            if result is True:
                message+='\n----(28) is a perfect number -Pass'
                test_passed(results_dict)
            else:
                message+='\n----The returned value is incorrect -Fail'
                test_failed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.check_is_less_than function - Fail'
        test_failed(results_dict)
...
def run():
    ...
    test_sorting(results_dict)
    test_perfect_num(results_dict)
